...
if($('.timeP').length){	
	...
	if(oTime-oPerTime>0){
        ...
        setTimeout(function(){
            ...
            if(oneLoat_p==0){
                var oHtml="<a href='"+$('.signal dl dd').eq(0).find('a').attr('href')+"' target='_blank'><div style='height:100%;padding:5px;text-align:center; position:relative'><span style='color:#999999'>外部站点赛事，点击跳出按钮观看</span><p style='text-align:center; display: flex; align-items: center; justify-content: center;height:40px;width:150px;color:#fff;-moz-border-radius:4px;-webkit-border-radius:4px;border-radius:4px; position:absolute;top:230%; left:50%;transform: translate(-50%, -50%); background:#0094bc'>点击跳出</p></div></a>"
                $('.nogame').append(oHtml);
            }else {
               // $('.nogame').show();
		       // $('.gaming').hide();
                //$(".nogame").append("<div style='width: 100%; height: 100%; position: relative'><p><img src='/~static/www/img/video.png' style='width:70px; height:70px; position: absolute; left: 50%;top: 50%;transform: translate(-50%,-50%);'></p></div>");
            }
...
function searchLink(oVa){
    var oDress="http://"+window.location.host+"/topic/so.php";
    $.ajax({
        url:oDress,
        type: "POST",
        cache:false,
        ...
        success:function(r){
            if(r.id){
                ...
                newWindow.location="/topic/" + r.id + ".html";
            }else{
                ...
                success:function(r){
                    ...
                    if(r.length){
                        for(i=0;i<r.length;i++){
                            var oHtml="<li><a target='_blank' href='/topic/" + r[i].id + ".html'>" + r[i].name + "</a></li>";
