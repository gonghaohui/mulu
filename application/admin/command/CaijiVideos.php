...
class CaijiVideos extends Command{
    //每次采集的条数
    private $num = 5;
    private $type = 3;//type 1:新闻2直播:3录像:4:集锦

    protected function configure(){
        $this->setName('caiji_videos')->setDescription("计划任务-采集录像");
    }

    protected function execute(Input $input, Output $output){
        $output->writeln('录像采集开始执行-'.date('Y-m-d H:i:s'));
        //判断是否允许采集
        $allow = Db::name('mulu_systems')->where('key','caiji_video')->find();
        if($allow['value'] == 0){
            $output->writeln('录像采集没有开启，结束采集...');
            exit;
        }
        $setting_array = Db::name('mulu_systems')->where('key','caiji_video_database_config')->find();
        $setting = json_decode($setting_array['value'],true);
        $conn = @mysqli_connect($setting['dbhost'],$setting['dbuser'],$setting['dbpwd']);
        if(empty($conn)){
            $output->writeln('录像远程数据连接失败');
            exit;
        }
        $db = mysqli_select_db($conn,$setting['dbname']);
        if(!$db){
            $output->writeln('录像远程数据库不存在');
            exit;
        }
...
//                    echo '表不存在';exit;
                ...
                foreach ($lists as $item){
                    if($item['biaoti'] == '' or $item['neirong'] == ''){
                        ...
                        $video_id = Db::name('mulu_videos')->insertGetId($input_data);
                        //标志这条记录发布成功
                        $remote_db->table($caiji_biao_name)->where('Id',$item['Id'])->update([
                            'fabu' => 1
                        ]);
                        if($video_id){
                            ...
                            if($biao['tag_id'] != 0){
                                ...
                                //++
                                if($rel){
                                    Db::name('mulu_new_tags')->where('new_tag_id',$biao['tag_id'])->setInc('video_count');
                                }
                            }
                            //上级
                            if($biao['pid'] != 0){
                                ...
                                if($parent_category and isset($parent_category['tag_id']) and $parent_category['tag_id'] != 0){
                                    ...
                                    //++
                                    if($rel){
                                        Db::name('mulu_new_tags')->where('new_tag_id',$parent_category['tag_id'])->setInc('video_count');
                                    }
...
//                                        echo "包含";
                                        ...
                                        //++
                                        if($rel){
                                            Db::name('mulu_new_tags')->where('new_tag_id',$team['tag_id'])->setInc('video_count');
                                        }
                                    }
        ...
        $output->writeln('录像采集结束-'.date('Y-m-d H:i:s'));
