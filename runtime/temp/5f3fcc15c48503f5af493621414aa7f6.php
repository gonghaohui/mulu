<?php if (!defined('THINK_PATH')) exit(); /*a:3:{s:69:"D:\gary\mulu\public/../application/admin\view\news\category_list.html";i:1670705547;s:54:"D:\gary\mulu\application\admin\view\public\header.html";i:1628315520;s:54:"D:\gary\mulu\application\admin\view\public\footer.html";i:1628864820;}*/ ?>
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
...
<div class="layui-fluid">
    <div class="layui-card">
        <div class="layui-form layui-card-header layuiadmin-card-header-auto">
            ...
            <div>
                <button class="layui-btn" data-type="add" onclick="wk.href('<?php echo url('add_category'); ?>')">
        ...
        <div class="layui-card-body">
            ...
            <script type="text/html" id="staBar">
                <input type="checkbox" value="{{d.category_id}}" name="status"  lay-skin="switch"  lay-text="ON|OFF" lay-filter="OnOff"{{ d.status == 1 ? 'checked' : '' }}  >
            </script>
            <script type="text/html" id="showNavigation">
                <input type="checkbox" value="{{d.category_id}}" name="show_navigation"  lay-skin="switch"  lay-text="ON|OFF" lay-filter="OnOff"{{ d.show_navigation == 1 ? 'checked' : '' }}  >
            </script>
            <script type="text/html" id="isCaiji">
                <input type="checkbox" value="{{d.category_id}}" name="is_caiji"  lay-skin="switch"  lay-text="ON|OFF" lay-filter="OnOff"{{ d.is_caiji == 1 ? 'checked' : '' }}  >
            </script>
            <script type="text/html" id="isSupport">
                <input type="checkbox" value="{{d.category_id}}" name="support"  lay-skin="switch"  lay-text="ON|OFF" lay-filter="OnOff"{{ d.support == 1 ? 'checked' : '' }}  >
            </script>

            <!--操作模板-->
            <script type="text/html" id="opeBar">
                <a class="layui-btn layui-btn-info layui-btn-xs" title="添加子分类" onclick="wk.href('<?php echo url('add_category'); ?>?id={{d.category_id}}')"><i class="fa fa-sitemap"></i></a>
                <a class="layui-btn layui-btn-xs" title="编辑" onclick="wk.href('<?php echo url('edit_category'); ?>?id={{d.category_id}}')"><i class="fa fa-pencil"></i></a>
...
<script>
    layui.use(['index', 'table','util'], function () {
        ...
        table.render({
            ...
            , url: '<?php echo url("news/category_list"); ?>'
            ,response: {
            ...
            , id: 'LAY-table'
            , cols: [[
                {field:'category_id', width: 80, title: 'ID号', align: 'center'}
                , {field: 'category_name', width: '' ,title: '分类名称', align: 'left',edit: 'text',templet:'<div>{{d.placeholder}}{{d.category_name}}</div>'},
                {field: 'pinyin',fixed: 'right', width: 250, title: '拼音URL', align: 'left'},
                ...
                {field: 'type',fixed: 'right', width: 80, title: '大分类', align: 'center',templet:function (d){
                    if(d.type == 1){
                        return '新闻';
                    }else if(d.type == 2){
                        return '直播';
                ...
                , {field: 'sort',fixed: 'right', width: 80, title: '排序', align: 'center',edit: 'text'}
                // , {field: 'type',fixed: 'right', width: 100, title: '类型', align: 'center', templet: '#typeTpl'}
                //, {field: 'status',fixed: 'right', width: 100, title: '状态', align: 'center', templet: '#staBar'}
                ,{field: 'caiji_biao_name', width: 150 ,title: '远程采集表名', align: 'left',edit: 'text',templet:'<div>{{d.caiji_biao_name}}</div>'}
                ,{field: 'is_caiji',fixed: 'right', width: 100, title: '是否采集', align: 'center', templet: '#isCaiji'}
                ,{field: 'support',fixed: 'right', width: 100, title: '推荐', align: 'center', templet: '#isSupport'}
        ...
        form.on('switch(OnOff)', function (obj) {
            ...
            //菜单状态
            wk.status(this.value,num, '<?php echo url("status_state"); ?>',obj);
        });
        ...
        table.on('edit(LAY-table-manage)', function(obj){
            ...
            wk.change(data.category_id,"<?php echo url('editField'); ?>",field,value,'mulu_category');
        });
    ...
    function del_category(obj,id){
        ...
        layer.confirm('删除前先把该分类下的分类删除,确认删除吗?', {icon: 7, title:'警告'}, function(index){
            //do something
            $.getJSON('<?php echo url("del_category"); ?>', {'id' : id}, function(res){
                if(res.code == 200){
